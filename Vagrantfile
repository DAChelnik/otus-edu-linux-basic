...
if File.file?("#{current_dir}/nodes/config/vagrant-config-nodes.yaml") && File.file?("#{current_dir}/nodes/config/vagrant-config-global.yaml")
  ...
  raise "Конфигурационные файлы отсутствуют"
end

# ПРОВЕРЯЕМ, УСТАНОВЛЕН ЛИ ПЛАГИН VAGRANT-DISKSIZE
unless Vagrant.has_plugin?("vagrant-disksize")
  raise "Плагин vagrant-disksize не установлен"
end
  ...
  confignodes["nodes"].each do |confignodes|
    if confignodes["setup"] == true
      config.vm.define confignodes["name"] do |vm|
        ...
        vm.vm.hostname = "#{confignodes["DACheF_domain_prefix"]}#{configglobal["GLOBAL"]["DACheF_domain_suffix"]}"
        ...
        if confignodes["document_root"]
          ...
          vm.vm.synced_folder "data-www/#{confignodes["name"]}/", confignodes["apache_document_root"],
            create: true, 
            owner: "www-data", 
            group: "www-data"
        end
        vm.vm.synced_folder "data-backup/#{confignodes["name"]}/", "/backup", 
          create: true
        vm.vm.synced_folder "data-tmp/#{confignodes["name"]}/", "/tmp/src", 
          create: true
        ...
        #     при каждой загрузке:
        #        ..      
        if confignodes["name"] == "ns"
        elsif confignodes["name"] == "mysql"
          # ТРИГГЕРЫ:
          vm.trigger.before :halt do |trigger|
            trigger.name = "\e[33m vagrant halt #{confignodes["name"]} VM\e[0m"
            trigger.info = "\n\e[33m Останавливаем MySQL сервер баз данных \e[0m"
            trigger.run_remote  = {path: "#{current_dir}/nodes/roles/#{confignodes["name"]}/vagrant-halt-mysql-before.sh"}
          end
          #
          # ПРОБРОС ПОРТОВ:
          ...
          vm.vm.network "forwarded_port", guest: confignodes["mysql_guest"], host: confignodes["mysql_host"]
          #
          # ДОПОЛНИТЕЛЬНАЯ КОНФИГУРАЦИЯ ВИРТУАЛЬНОЙ МАШИНЫ С ПОМОЩЬЮ SHELL
          if File.file?("#{current_dir}/nodes/roles/#{confignodes["name"]}/createdb-mysql.sh")
            confignodes["mysql_database"].each do |configmysql|
              vm.vm.provision "shell", path: "#{current_dir}/nodes/roles/#{confignodes["name"]}/createdb-mysql.sh",
                env: {
                  "DB_ROOT_USER"    => "#{confignodes["mysql_dbuser"]}", 
                  "DB_ROOT_PASSWD"  => "#{confignodes["mysql_dbpasswd"]}",
        ...
        elsif confignodes["name"] == "production"
          #   ДОПОЛНИТЕЛЬНАЯ КОНФИГУРАЦИЯ ВИРТУАЛЬНОЙ МАШИНЫ С ПОМОЩЬЮ SHELL
          if confignodes["web_based_interface"] == "virtualmin"
            ...
            configglobal["GLOBAL"]["VirtualHost"].each do |virtualhost|
              vm.vm.provision "shell", path: "#{current_dir}/nodes/vprovision/irtualhost-create.sh",
                env: {
        ...
        elsif confignodes["name"] == "wiki"
          vm.vm.provision "shell", path: "#{current_dir}/nodes/roles/#{confignodes["name"]}/wiki.sh",
            env: {
              "ServerAdmin"   => "#{confignodes["ServerAdmin"]}",
              "DOMAIN"        => "#{confignodes["ServerName"]}",
              "ServerAlias"   => "#{virtualhost["ServerAlias"]}",
              "DocumentRoot"  => "#{virtualhost["DocumentRoot"]}",
            }
        end                     
